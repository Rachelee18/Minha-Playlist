...
function PlaylistEditor() {
  ...
  const handleOnDragEnd = (result) => {
    ...
    if (
      ...
      setAllMusics(allMusics.filter((_, i) => i !== source.index));
    }
  };
  ...
  return (
    <DragDropContext onDragEnd={handleOnDragEnd}>
      <div className="w-[100vw] h-[100vh] bg-gray-50">
              ...
              {playlistMusics.length === 0 ? (
                ...
                <div className="w-full p-4">
                  {playlistMusics.map((music, index) => (
                    <div
                      key={`${music.id}-${index}`}
                      className="flex justify-between items-center p-3 mb-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition-all"
                    >
        ...
        <div className="w-[1910px] h-36 rounded-xl shadow-[-5px_15px_10px_20px_rgba(1,0,0,0.1)] absolute justify-center items-center left-1/2 -translate-x-1/2 bottom-2 transition-all">
